...
import { getPromptById, updatePromptById } from 'apiSdk/prompts';
...
function PromptEditPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<PromptInterface>(
    () => (id ? `/prompts/${id}` : null),
    () => getPromptById(id),
  );
  ...
  const handleSubmit = async (values: PromptInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updatePromptById(id, values);
      mutate(updated);
      resetForm();
      router.push('/prompts');
    } catch (error) {
